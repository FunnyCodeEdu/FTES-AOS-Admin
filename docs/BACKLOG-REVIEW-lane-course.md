...
| admin-course-challenge-bank | Toggle visibility — loading state (CourseChallengeBankTab.tsx) | `setVisibility.isPending` là state dùng chung của 1 mutation → khi toggle 1 row, TẤT CẢ nút visibility ở mọi row cùng hiện `loading`. Cosmetic, không sai logic (mutateAsync theo id đúng). | Track id đang mutate (`useState<string>`) để chỉ nút của row đó loading, hoặc disable toàn bảng bằng overlay. |
